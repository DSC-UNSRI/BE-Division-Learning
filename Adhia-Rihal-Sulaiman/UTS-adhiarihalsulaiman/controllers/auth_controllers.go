...
func Register(w http.ResponseWriter, r *http.Request) {
	...
	_, err = database.DB.Exec("INSERT INTO forgets (user_id, question, answer_hash) VALUES (?, ?, ?)",
...
func Login(w http.ResponseWriter, r *http.Request) {
	...
	err := database.DB.QueryRow("SELECT user_id, username, password, role, type FROM users WHERE username = ? AND deleted_at IS NULL", username).
		Scan(&user.ID, &user.Username, &user.Password, &user.Role, &user.Type)
	if err != nil {
	...
	_, err = database.DB.Exec("INSERT INTO tokens (user_id, token_value, expires_at) VALUES (?, ?, ?)",
		user.ID, newToken, expirationDate,
	)
...
func InitiatePasswordReset(w http.ResponseWriter, r *http.Request) {
	...
	var forgetQuestion string

	err = database.DB.QueryRow("SELECT question FROM forgets WHERE user_id = ?", userID).Scan(&forgetQuestion)
...
func PasswordReset(w http.ResponseWriter, r *http.Request) {
	...
	err := database.DB.QueryRow(
		"SELECT u.user_id, f.answer_hash FROM users u JOIN forgets f ON u.user_id = f.user_id WHERE u.username = ? AND u.deleted_at IS NULL",
		username,
