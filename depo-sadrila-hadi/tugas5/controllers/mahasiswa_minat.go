...
func AddMinatToMahasiswa(w http.ResponseWriter, r *http.Request, mahasiswaIDStr string) {
	...
	var mhsExists bool
	err = database.DB.QueryRow("SELECT EXISTS(SELECT 1 FROM mahasiswa WHERE id = ? AND deleted_at IS NULL)", mahasiswaID).Scan(&mhsExists)
	...
	_, err = database.DB.Exec(query, mahasiswaID, input.MinatID)
	if err != nil {
...
func GetMinatByMahasiswaID(w http.ResponseWriter, r *http.Request, mahasiswaIDStr string) {
	...
	`
	rows, err := database.DB.Query(query, mahasiswaID)
	if err != nil {
...
func RemoveMinatFromMahasiswa(w http.ResponseWriter, r *http.Request, mahasiswaIDStr string, minatIDStr string) {
	...
	res, err := database.DB.Exec(query, mahasiswaID, minatID)
	if err != nil {
